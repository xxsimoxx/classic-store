...
abstract class WC_REST_Posts_Controller extends WC_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
			...
			return new WP_Error( "woocommerce_rest_invalid_{$this->post_type}_id", __( 'Invalid ID.', 'classic-store'), array( 'status' => 404 ) );
		}
	...
	public function get_collection_params() {
		...
		if ( 'wc/v1' === $this->namespace ) {
			$params['filter'] = array(
				'type'        => 'object',
				'description' => __( 'Use WP Query arguments to modify the response; private query vars require appropriate authorization.', 'classic-store'),
			);
		}
