...
function wc_rest_upload_image_from_url( $image_url ) {
	...
	if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
		...
		return new WP_Error( 'woocommerce_rest_invalid_image_url', sprintf( __( 'Invalid URL %s.', 'classic-store'), $image_url ), array( 'status' => 400 ) );
	}
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error( 'woocommerce_rest_invalid_remote_image_url',
			/* translators: %s: image URL */
			sprintf( __( 'Error getting remote image %s.', 'classic-store'), $image_url ) . ' '
			/* translators: %s: error message */
			. sprintf( __( 'Error: %s.', 'classic-store'), $response->get_error_message() ), array( 'status' => 400 )
		);
	} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
		/* translators: %s: image URL */
		return new WP_Error( 'woocommerce_rest_invalid_remote_image_url', sprintf( __( 'Error getting remote image %s.', 'classic-store'), $image_url ), array( 'status' => 400 ) );
	}
	...
	if ( ! $wp_filetype['type'] ) {
		...
		if ( ! $wp_filetype['type'] ) {
			return new WP_Error( 'woocommerce_rest_invalid_image_type', __( 'Invalid image type.', 'classic-store'), array( 'status' => 400 ) );
		}
	}
	...
	if ( ! $filesize ) {
		...
		return new WP_Error( 'woocommerce_rest_image_upload_file_error', __( 'Zero size file downloaded.', 'classic-store'), array( 'status' => 400 ) );
	}
...
function wc_rest_validate_reports_request_arg( $value, $request, $param ) {
	...
	if ( 'string' === $args['type'] && ! is_string( $value ) ) {
		...
		return new WP_Error( 'woocommerce_rest_invalid_param', sprintf( __( '%1$s is not of type %2$s', 'classic-store'), $param, 'string' ) );
	}
	...
	if ( 'date' === $args['format'] ) {
		...
		if ( ! preg_match( $regex, $value, $matches ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_date', __( 'The date you provided is invalid.', 'classic-store') );
		}
	}
