...
class WC_Admin_API_Keys {
		...
		if ( isset( $_GET['create-key'] ) || isset( $_GET['edit-key'] ) ) {
			...
			if ( $key_id && $user_id && ! current_user_can( 'edit_user', $user_id ) ) {
				if ( get_current_user_id() !== $user_id ) {
					wp_die( esc_html__( 'You do not have permission to edit this API Key', 'classic-store') );
				}
			}
	...
	private static function table_list_output() {
		...
		echo '<h2>' . esc_html__( 'REST API', 'classic-store') . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys&create-key=1' ) ) . '" class="add-new-h2">' . esc_html__( 'Add key', 'classic-store') . '</a></h2>';
		...
		if ( absint( $count ) && $count > 0 ) {
			...
			$keys_table_list->search_box( __( 'Search key', 'classic-store'), 'key' );
			$keys_table_list->display();
		} else {
			echo '<div class="woocommerce-BlankState woocommerce-BlankState--api">';
			?>
			<h2 class="woocommerce-BlankState-message"><?php esc_html_e( 'The Classic Commerce REST API allows external apps to view and manage store data. Access is granted only to those with valid API keys.', 'classic-store'); ?></h2>
			<a class="woocommerce-BlankState-cta button-primary button" href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys&create-key=1' ) ); ?>"><?php esc_html_e( 'Create an API key', 'classic-store'); ?></a>
	...
	public static function notices() {
		if ( isset( $_GET['revoked'] ) ) { // WPCS: input var okay, CSRF ok.
			...
			/* translators: %d: count */
			WC_Admin_Settings::add_message( sprintf( _n( '%d API key permanently revoked.', '%d API keys permanently revoked.', $revoked, 'classic-store' ), $revoked ) );
		}
	}
	...
	private function revoke_key() {
		...
		if ( isset( $_REQUEST['revoke-key'] ) ) { // WPCS: input var okay, CSRF ok.
			...
			if ( $key_id && $user_id && ( current_user_can( 'edit_user', $user_id ) || get_current_user_id() === $user_id ) ) {
				...
				wp_die( esc_html__( 'You do not have permission to revoke this API Key', 'classic-store') );
			}
		}
	...
	private function bulk_actions() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to edit API Keys', 'classic-store') );
		}
	...
	private function bulk_revoke_key( $keys ) {
		if ( ! current_user_can( 'remove_users' ) ) {
			wp_die( esc_html__( 'You do not have permission to revoke API Keys', 'classic-store') );
		}
