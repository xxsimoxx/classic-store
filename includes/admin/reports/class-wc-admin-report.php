...
class WC_Admin_Report {
	...
	public function sales_sparkline( $id = '', $days = 7, $type = 'sales' ) {
		...
		if ( 'sales' === $type ) {
			...
			$tooltip = sprintf( __( 'Sold %1$s worth in the last %2$d days', 'classic-store'), strip_tags( wc_price( $total ) ), $days );
		} else {
			/* translators: 1: total items sold 2: days */
			$tooltip = sprintf( _n( 'Sold %1$d item in the last %2$d days', 'Sold %1$d items in the last %2$d days', $total, 'classic-store' ), $total, $days );
		}
	...
	public function check_current_range_nonce( $current_range ) {
		...
		if ( ! isset( $_GET['wc_reports_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['wc_reports_nonce'] ), 'custom_range' ) ) { // WPCS: input var ok, CSRF ok.
			wp_die(
				...
				sprintf( esc_html__( 'This report link has expired. %1$sClick here to view the filtered report%2$s.', 'classic-store'), '<a href="' . esc_url( wp_nonce_url( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ), 'custom_range', 'wc_reports_nonce' ) ) . '">', '</a>' ), // @codingStandardsIgnoreLine.
				esc_attr__( 'Confirm navigation', 'classic-store')
			);
			exit;
		}
	}
