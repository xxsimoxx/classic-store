...
class WC_Admin_List_Table_Coupons extends WC_Admin_List_Table {
	...
	protected function render_blank_state() {
		...
		echo '<h2 class="woocommerce-BlankState-message">' . esc_html__( 'Coupons are a great way to offer discounts and rewards to your customers. They will appear here once created.', 'classic-store') . '</h2>';
		echo '<a class="woocommerce-BlankState-cta button-primary button" href="' . esc_url( admin_url( 'post-new.php?post_type=shop_coupon' ) ) . '">' . esc_html__( 'Create your first coupon', 'classic-store') . '</a>';
		echo '<a class="woocommerce-BlankState-cta button" target="_blank" href="https://classiccommerce.cc/docs/installation-and-setup/coupons/">' . esc_html__( 'Learn more about coupons', 'classic-store') . '</a>';
		echo '</div>';
	}
	...
	public function define_columns( $columns ) {
		...
		$show_columns['coupon_code'] = __( 'Code', 'classic-store');
		$show_columns['type']        = __( 'Coupon type', 'classic-store');
		$show_columns['amount']      = __( 'Coupon amount', 'classic-store');
		$show_columns['description'] = __( 'Description', 'classic-store');
		$show_columns['products']    = __( 'Product IDs', 'classic-store');
		$show_columns['usage']       = __( 'Usage / Limit', 'classic-store');
		$show_columns['expiry_date'] = __( 'Expiry date', 'classic-store');
	...
	protected function render_filters() {
		...
		<select name="coupon_type" id="dropdown_shop_coupon_type">
			<option value=""><?php esc_html_e( 'Show all types', 'classic-store'); ?></option>
			<?php
