...
class WC_Admin_Meta_Boxes {
	...
	public function add_meta_boxes() {
		...
		// Products.
		add_meta_box( 'postexcerpt', __( 'Product short description', 'classic-store'), 'WC_Meta_Box_Product_Short_Description::output', 'product', 'normal' );
		add_meta_box( 'woocommerce-product-data', __( 'Product data', 'classic-store'), 'WC_Meta_Box_Product_Data::output', 'product', 'normal', 'high' );
		add_meta_box( 'woocommerce-product-images', __( 'Product gallery', 'classic-store'), 'WC_Meta_Box_Product_Images::output', 'product', 'side', 'low' );

		// Orders.
		foreach ( wc_get_order_types( 'order-meta-boxes' ) as $type ) {
			$order_type_object = get_post_type_object( $type );
            /* Translators: %s order type name. */
			add_meta_box( 'woocommerce-order-data', sprintf( __( '%s data', 'classic-store'), $order_type_object->labels->singular_name ), 'WC_Meta_Box_Order_Data::output', $type, 'normal', 'high' );
			add_meta_box( 'woocommerce-order-items', __( 'Items', 'classic-store'), 'WC_Meta_Box_Order_Items::output', $type, 'normal', 'high' );
            /* Translators: %s order type name. */
			add_meta_box( 'woocommerce-order-notes', sprintf( __( '%s notes', 'classic-store'), $order_type_object->labels->singular_name ), 'WC_Meta_Box_Order_Notes::output', $type, 'side', 'default' );
			add_meta_box( 'woocommerce-order-downloads', __( 'Downloadable product permissions', 'classic-store') . wc_help_tip( __( 'Note: Permissions for order items will automatically be granted when the order status changes to processing/completed.', 'classic-store') ), 'WC_Meta_Box_Order_Downloads::output', $type, 'normal', 'default' );
            /* Translators: %s order type name. */
			add_meta_box( 'woocommerce-order-actions', sprintf( __( '%s actions', 'classic-store'), $order_type_object->labels->singular_name ), 'WC_Meta_Box_Order_Actions::output', $type, 'side', 'high' );
		}

		// Coupons.
		add_meta_box( 'woocommerce-coupon-data', __( 'Coupon data', 'classic-store'), 'WC_Meta_Box_Coupon_Data::output', 'shop_coupon', 'normal', 'high' );
		...
		// Comment rating.
			...
			add_meta_box( 'woocommerce-rating', __( 'Rating', 'classic-store'), 'WC_Meta_Box_Product_Reviews::output', 'comment', 'normal', 'high' );
		}
	}
	...
	public function rename_meta_boxes() {
		...
		if ( isset( $post ) && ( 'publish' === $post->post_status || 'private' === $post->post_status ) && post_type_supports( 'product', 'comments' ) ) {
			remove_meta_box( 'commentsdiv', 'product', 'normal' );
			add_meta_box( 'commentsdiv', __( 'Reviews', 'classic-store'), 'post_comment_meta_box', 'product', 'normal' );
		}
	}
