...
class WC_Meta_Box_Order_Actions {
		...
		<ul class="order_actions submitbox">
			...
			<li class="wide" id="actions">
				<select name="wc_order_action">
					<option value=""><?php esc_html_e( 'Choose an action...', 'classic-store'); ?></option>
					<?php foreach ( $order_actions as $action => $title ) { ?>
						<option value="<?php echo esc_attr( $action ); ?>"><?php echo esc_html( $title ); ?></option>
					<?php } ?>
				</select>
				<button class="button wc-reload"><span><?php esc_html_e( 'Apply', 'classic-store'); ?></span></button>
			</li>
			...
			<li class="wide">
				<div id="delete-action">
					...
					if ( current_user_can( 'delete_post', $post->ID ) ) {
						...
						if ( ! EMPTY_TRASH_DAYS ) {
							$delete_text = __( 'Delete permanently', 'classic-store');
						} else {
							$delete_text = __( 'Move to Trash', 'classic-store');
						}
						?>
				...
				<button type="submit" class="button save_order button-primary" name="save" value="<?php echo 'auto-draft' === $post->post_status ? esc_attr__( 'Create', 'classic-store') : esc_attr__( 'Update', 'classic-store'); ?>"><?php echo 'auto-draft' === $post->post_status ? esc_html__( 'Create', 'classic-store') : esc_html__( 'Update', 'classic-store'); ?></button>
			</li>
	...
	public static function save( $post_id, $post ) {
		...
		if ( ! empty( $_POST['wc_order_action'] ) ) { // @codingStandardsIgnoreLine
			...
			if ( 'send_order_details' === $action ) {
				...
				// Note the event.
				$order->add_order_note( __( 'Order details manually sent to customer.', 'classic-store'), false, true );
	...
	private static function get_available_order_actions_for_order( $order ) {
		$actions = array(
			'send_order_details'              => __( 'Send order details to customer', 'classic-store'),
			'send_order_details_admin'        => __( 'Resend new order notification', 'classic-store'),
			'regenerate_download_permissions' => __( 'Regenerate download permissions', 'classic-store'),
		);
