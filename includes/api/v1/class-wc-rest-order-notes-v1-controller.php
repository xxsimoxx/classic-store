...
class WC_REST_Order_Notes_V1_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid order ID.', 'classic-store'), array( 'status' => 404 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'woocommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'classic-store'), 'order_note' ), array( 'status' => 500 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'note' => array(
					'description' => __( 'Order note.', 'classic-store'),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'customer_note' => array(
					'description' => __( 'Shows/define if the note is only for reference or for the customer (the user will be notified).', 'classic-store' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
