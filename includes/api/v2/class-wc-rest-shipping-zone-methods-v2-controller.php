...
class WC_REST_Shipping_Zone_Methods_V2_Controller extends WC_REST_Shipping_Zones_Controller_Base {
	...
	public function register_routes() {
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<zone_id>[\d]+)/methods', array(
				'args'   => array(
					'zone_id' => array(
						'description' => __( 'Unique ID for the zone.', 'classic-store'),
						'type'        => 'integer',
					),
				),
				array(
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<zone_id>[\d]+)/methods/(?P<instance_id>[\d]+)', array(
				'args'   => array(
					...
					'instance_id' => array(
						'description' => __( 'Unique ID for the instance.', 'classic-store'),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_item( $request ) {
		...
		if ( false === $method ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_method_invalid', __( 'Resource does not exist.', 'classic-store'), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( false === $method ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_not_created', __( 'Resource cannot be created.', 'classic-store'), array( 'status' => 500 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( $force ) {
			...
			return new WP_Error( 'rest_trash_not_supported', __( 'Shipping methods do not support trashing.', 'classic-store'), array( 'status' => 501 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'                 => array(
					'description' => __( 'Shipping method instance ID.', 'classic-store'),
					'type'        => 'integer',
				...
				'title'              => array(
					'description' => __( 'Shipping method customer facing title.', 'classic-store'),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'order'              => array(
					'description' => __( 'Shipping method sort order.', 'classic-store'),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'enabled'            => array(
					'description' => __( 'Shipping method enabled status.', 'classic-store'),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
				),
				'method_id'          => array(
				...
				'method_title'       => array(
					'description' => __( 'Shipping method title.', 'classic-store'),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'method_description' => array(
					'description' => __( 'Shipping method description.', 'classic-store'),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'settings'           => array(
					'description' => __( 'Shipping method settings.', 'classic-store'),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id'          => array(
