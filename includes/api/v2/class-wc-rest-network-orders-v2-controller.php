...
class WC_REST_Network_Orders_V2_Controller extends WC_REST_Orders_V2_Controller {
	...
	public function get_public_item_schema() {
		...
		$schema['properties']['blog']              = array(
			'description' => __( 'Blog id of the record on the multisite.', 'classic-store'),
			'type'        => 'integer',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		$schema['properties']['edit_url']          = array(
			'description' => __( 'URL to edit the order', 'classic-store'),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		$schema['properties']['customer'][]        = array(
			'description' => __( 'Name of the customer for the order', 'classic-store'),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		...
		$schema['properties']['formatted_total'][] = array(
			'description' => __( 'Order total formatted for locale', 'classic-store'),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
	...
	public function network_orders( $request ) {
		...
		foreach ( $items->data as &$current_order ) {
			...
			$current_order['customer']        = trim( sprintf( _x( '%1$s %2$s', 'full name', 'classic-store'), $order->get_billing_first_name(), $order->get_billing_last_name() ) );
