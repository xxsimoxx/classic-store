...
class WC_REST_Product_Variations_V2_Controller extends WC_REST_Products_V2_Controller {
	...
	public function register_routes() {
		register_rest_route(
			$this->namespace, '/' . $this->rest_base, array(
				'args'   => array(
					'product_id' => array(
						'description' => __( 'Unique identifier for the variable product.', 'classic-store'),
						'type'        => 'integer',
					),
				),
				array(
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<id>[\d]+)', array(
				'args'   => array(
					...
					'id'         => array(
						'description' => __( 'Unique identifier for the variation.', 'classic-store'),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function delete_item( $request ) {
		...
		if ( ! $object || 0 === $object->get_id() ) {
			return new WP_Error(
				"woocommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'classic-store'), array(
					'status' => 404,
				)
			);
		}
		...
		if ( ! wc_rest_check_post_permissions( $this->post_type, 'delete', $object->get_id() ) ) {
			return new WP_Error(
				...
				"woocommerce_rest_user_cannot_delete_{$this->post_type}", sprintf( __( 'Sorry, you are not allowed to delete %s.', 'classic-store'), $this->post_type ), array(
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				return new WP_Error(
					/* translators: %s: post type */
					'woocommerce_rest_trash_not_supported', sprintf( __( 'The %s does not support trashing.', 'classic-store'), $this->post_type ), array(
						'status' => 501,
					)
				);
			}
			...
			if ( is_callable( array( $object, 'get_status' ) ) ) {
				if ( 'trash' === $object->get_status() ) {
					return new WP_Error(
						/* translators: %s: post type */
						'woocommerce_rest_already_trashed', sprintf( __( 'The %s has already been deleted.', 'classic-store'), $this->post_type ), array(
							'status' => 410,
						)
					);
				}
		...
		if ( ! $result ) {
			return new WP_Error(
				/* translators: %s: post type */
				'woocommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'classic-store'), $this->post_type ), array(
					'status' => 500,
				)
			);
		}
	...
	public function get_item_schema() {
		...
		$schema         = array(
			...
			'properties' => array(
				...
				'description'           => array(
					'description' => __( 'Variation description.', 'classic-store'),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'permalink'             => array(
				...
				'visible'               => array(
					'description' => __( "Define if the variation is visible on the product's page.", 'classic-store'),
					'type'        => 'boolean',
					'default'     => true,
					'context'     => array( 'view', 'edit' ),
				),
