...
class WC_REST_Shipping_Zones_V2_Controller extends WC_REST_Shipping_Zones_Controller_Base {
					...
					'args'                => array_merge(
						$this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
							'name' => array(
								'required'    => true,
								'type'        => 'string',
								'description' => __( 'Shipping zone name.', 'classic-store'),
							),
						)
					),
				),
	...
	public function create_item( $request ) {
		...
		if ( $zone->get_id() !== 0 ) {
			...
			return new WP_Error( 'woocommerce_rest_shipping_zone_not_created', __( "Resource cannot be created. Check to make sure 'order' and 'name' are present.", 'classic-store'), array( 'status' => 500 ) );
		}
	}
	...
	public function update_item( $request ) {
		...
		if ( 0 === $zone->get_id() ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_invalid_zone', __( 'The "locations not covered by your other zones" zone cannot be updated.', 'classic-store'), array( 'status' => 403 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( $force ) {
			...
			return new WP_Error( 'rest_trash_not_supported', __( 'Shipping zones do not support trashing.', 'classic-store'), array( 'status' => 501 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'order' => array(
					'description' => __( 'Shipping zone order.', 'classic-store'),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
			),
