...
abstract class WC_REST_CRUD_Controller extends WC_REST_Posts_Controller {
	...
	protected function get_object( $id ) {
		...
		return new WP_Error( 'invalid-method', sprintf( __( "Method '%s' not implemented. Must be overridden in subclass.", 'classic-store'), __METHOD__ ), array( 'status' => 405 ) );
	}
	...
	public function update_item( $request ) {
		...
		if ( ! $object || 0 === $object->get_id() ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'classic-store'), array( 'status' => 400 ) );
		}
	...
	public function get_collection_params() {
		...
		$params['page']            = array(
			'description'       => __( 'Current page of the collection.', 'classic-store'),
			'type'              => 'integer',
			'default'           => 1,
		...
		$params['per_page']        = array(
			'description'       => __( 'Maximum number of items to be returned in result set.', 'classic-store'),
			'type'              => 'integer',
			'default'           => 10,
			'minimum'           => 1,
			'maximum'           => 100,
		...
		$params['search']          = array(
			'description'       => __( 'Limit results to those matching a string.', 'classic-store'),
			'type'              => 'string',
		...
		$params['modified_after']  = array(
			'description'       => __( 'Limit response to resources modified after a given ISO8601 compliant date.', 'classic-store'),
			'type'              => 'string',
			'format'            => 'date-time',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['modified_before'] = array(
			'description'       => __( 'Limit response to resources modified before a given ISO8601 compliant date.', 'classic-store'),
			'type'              => 'string',
			'format'            => 'date-time',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['dates_are_gmt']   = array(
			'description'       => __( 'Whether to consider GMT post dates when limiting response by published or modified date.', 'classic-store'),
			'type'              => 'boolean',
			'default'           => false,
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		if ( $this->hierarchical ) {
			$params['parent']         = array(
				'description'       => __( 'Limit result set to those of particular parent IDs.', 'classic-store'),
				'type'              => 'array',
				'items'             => array(
					'type' => 'integer',
				),
			...
			$params['parent_exclude'] = array(
				'description'       => __( 'Limit result set to all items except those of a particular parent ID.', 'classic-store'),
				'type'              => 'array',
				'items'             => array(
					'type' => 'integer',
				),
