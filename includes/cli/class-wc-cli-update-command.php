...
class WC_CLI_Update_Command {
	...
	public static function update() {
		...
		foreach ( WC_Install::get_db_update_callbacks() as $version => $update_callbacks ) {
			if ( version_compare( $current_db_version, $version, '<' ) ) {
				foreach ( $update_callbacks as $update_callback ) {
					...
					WP_CLI::log( sprintf( __( 'Calling update function: %s', 'classic-store'), $update_callback ) );
		...
		WP_CLI::success( sprintf( __( '%1$d updates complete. Database version is %2$s', 'classic-store'), absint( $update_count ), get_option( 'woocommerce_db_version' ) ) );
	}
}
