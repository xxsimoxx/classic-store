...
class WC_Shortcode_Order_Tracking {
	...
	public static function output( $atts ) {
		...
		if ( isset( $_REQUEST['orderid'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-order_tracking' ) ) { // WPCS: input var ok.
			...
			if ( ! $order_id ) {
				wc_print_notice( __( 'Please enter a valid order ID', 'classic-store'), 'error' );
			} elseif ( ! $order_email ) {
				wc_print_notice( __( 'Please enter a valid email address', 'classic-store'), 'error' );
			} else {
				...
				if ( $order && $order->get_id() && strtolower( $order->get_billing_email() ) === strtolower( $order_email ) ) {
					...
					wc_print_notice( __( 'Sorry, the order could not be found. Please contact us if you are having difficulty finding your order details.', 'classic-store'), 'error' );
				}
			}
		}
