...
function wc_update_order( $args ) {
	if ( empty( $args['order_id'] ) ) {
		return new WP_Error( __( 'Invalid order ID.', 'classic-store') );
	}
...
function wc_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	...
	$template  = (string) wp_cache_get( $cache_key, 'classic-commerce' );
	...
	if ( $filter_template !== $template ) {
		if ( ! file_exists( $filter_template ) ) {
			...
			wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'classic-store'), '<code>' . $filter_template . '</code>' ), '2.1' );
			return;
		}
	...
	if ( ! empty( $args ) && is_array( $args ) ) {
		if ( isset( $args['action_args'] ) ) {
			wc_doing_it_wrong(
				__FUNCTION__,
				__( 'action_args should not be overwritten when calling wc_get_template.', 'classic-store'),
				'3.6.0'
			);
...
function wc_set_template_cache( $cache_key, $template ) {
	wp_cache_set( $cache_key, $template, 'classic-commerce' );

	$cached_templates = wp_cache_get( 'cached_templates', 'classic-commerce' );
	...
	wp_cache_set( 'cached_templates', $cached_templates, 'classic-commerce' );
}
...
function wc_clear_template_cache() {
	...
	if ( is_array( $cached_templates ) ) {
		foreach ( $cached_templates as $cache_key ) {
			wp_cache_delete( $cache_key, 'classic-commerce' );
		}
		wp_cache_delete( 'cached_templates', 'classic-commerce' );
	}
}
	...
	if ( ! isset( $currencies ) ) {
		$currencies = array_unique(
			apply_filters(
				...
				array(
					'AED' => __( 'United Arab Emirates dirham', 'classic-store'),
					'AFN' => __( 'Afghan afghani', 'classic-store'),
					'ALL' => __( 'Albanian lek', 'classic-store'),
					'AMD' => __( 'Armenian dram', 'classic-store'),
					'ANG' => __( 'Netherlands Antillean guilder', 'classic-store'),
					'AOA' => __( 'Angolan kwanza', 'classic-store'),
					'ARS' => __( 'Argentine peso', 'classic-store'),
					'AUD' => __( 'Australian dollar', 'classic-store'),
					'AWG' => __( 'Aruban florin', 'classic-store'),
					'AZN' => __( 'Azerbaijani manat', 'classic-store'),
					'BAM' => __( 'Bosnia and Herzegovina convertible mark', 'classic-store'),
					'BBD' => __( 'Barbadian dollar', 'classic-store'),
					'BDT' => __( 'Bangladeshi taka', 'classic-store'),
					'BGN' => __( 'Bulgarian lev', 'classic-store'),
					'BHD' => __( 'Bahraini dinar', 'classic-store'),
					'BIF' => __( 'Burundian franc', 'classic-store'),
					'BMD' => __( 'Bermudian dollar', 'classic-store'),
					'BND' => __( 'Brunei dollar', 'classic-store'),
					'BOB' => __( 'Bolivian boliviano', 'classic-store'),
					'BRL' => __( 'Brazilian real', 'classic-store'),
					'BSD' => __( 'Bahamian dollar', 'classic-store'),
					'BTC' => __( 'Bitcoin', 'classic-store'),
					'BTN' => __( 'Bhutanese ngultrum', 'classic-store'),
					'BWP' => __( 'Botswana pula', 'classic-store'),
					'BYR' => __( 'Belarusian ruble (old)', 'classic-store' ),
					'BYN' => __( 'Belarusian ruble', 'classic-store'),
					'BZD' => __( 'Belize dollar', 'classic-store'),
					'CAD' => __( 'Canadian dollar', 'classic-store'),
					'CDF' => __( 'Congolese franc', 'classic-store'),
					'CHF' => __( 'Swiss franc', 'classic-store'),
					'CLP' => __( 'Chilean peso', 'classic-store'),
					'CNY' => __( 'Chinese yuan', 'classic-store'),
					'COP' => __( 'Colombian peso', 'classic-store'),
					'CRC' => __( 'Costa Rican col&oacute;n', 'classic-store'),
					'CUC' => __( 'Cuban convertible peso', 'classic-store'),
					'CUP' => __( 'Cuban peso', 'classic-store'),
					'CVE' => __( 'Cape Verdean escudo', 'classic-store'),
					'CZK' => __( 'Czech koruna', 'classic-store'),
					'DJF' => __( 'Djiboutian franc', 'classic-store'),
					'DKK' => __( 'Danish krone', 'classic-store'),
					'DOP' => __( 'Dominican peso', 'classic-store'),
					'DZD' => __( 'Algerian dinar', 'classic-store'),
					'EGP' => __( 'Egyptian pound', 'classic-store'),
					'ERN' => __( 'Eritrean nakfa', 'classic-store'),
					'ETB' => __( 'Ethiopian birr', 'classic-store'),
					'EUR' => __( 'Euro', 'classic-store'),
					'FJD' => __( 'Fijian dollar', 'classic-store'),
					'FKP' => __( 'Falkland Islands pound', 'classic-store'),
					'GBP' => __( 'Pound sterling', 'classic-store'),
					'GEL' => __( 'Georgian lari', 'classic-store'),
					'GGP' => __( 'Guernsey pound', 'classic-store'),
					'GHS' => __( 'Ghana cedi', 'classic-store'),
					'GIP' => __( 'Gibraltar pound', 'classic-store'),
					'GMD' => __( 'Gambian dalasi', 'classic-store'),
					'GNF' => __( 'Guinean franc', 'classic-store'),
					'GTQ' => __( 'Guatemalan quetzal', 'classic-store'),
					'GYD' => __( 'Guyanese dollar', 'classic-store'),
					'HKD' => __( 'Hong Kong dollar', 'classic-store'),
					'HNL' => __( 'Honduran lempira', 'classic-store'),
					'HRK' => __( 'Croatian kuna', 'classic-store'),
					'HTG' => __( 'Haitian gourde', 'classic-store'),
					'HUF' => __( 'Hungarian forint', 'classic-store'),
					'IDR' => __( 'Indonesian rupiah', 'classic-store'),
					'ILS' => __( 'Israeli new shekel', 'classic-store'),
					'IMP' => __( 'Manx pound', 'classic-store'),
					'INR' => __( 'Indian rupee', 'classic-store'),
					'IQD' => __( 'Iraqi dinar', 'classic-store'),
					'IRR' => __( 'Iranian rial', 'classic-store'),
					'IRT' => __( 'Iranian toman', 'classic-store'),
					'ISK' => __( 'Icelandic kr&oacute;na', 'classic-store'),
					'JEP' => __( 'Jersey pound', 'classic-store'),
					'JMD' => __( 'Jamaican dollar', 'classic-store'),
					'JOD' => __( 'Jordanian dinar', 'classic-store'),
					'JPY' => __( 'Japanese yen', 'classic-store'),
					'KES' => __( 'Kenyan shilling', 'classic-store'),
					'KGS' => __( 'Kyrgyzstani som', 'classic-store'),
					'KHR' => __( 'Cambodian riel', 'classic-store'),
					'KMF' => __( 'Comorian franc', 'classic-store'),
					'KPW' => __( 'North Korean won', 'classic-store'),
					'KRW' => __( 'South Korean won', 'classic-store'),
					'KWD' => __( 'Kuwaiti dinar', 'classic-store'),
					'KYD' => __( 'Cayman Islands dollar', 'classic-store'),
					'KZT' => __( 'Kazakhstani tenge', 'classic-store'),
					'LAK' => __( 'Lao kip', 'classic-store'),
					'LBP' => __( 'Lebanese pound', 'classic-store'),
					'LKR' => __( 'Sri Lankan rupee', 'classic-store'),
					'LRD' => __( 'Liberian dollar', 'classic-store'),
					'LSL' => __( 'Lesotho loti', 'classic-store'),
					'LYD' => __( 'Libyan dinar', 'classic-store'),
					'MAD' => __( 'Moroccan dirham', 'classic-store'),
					'MDL' => __( 'Moldovan leu', 'classic-store'),
					'MGA' => __( 'Malagasy ariary', 'classic-store'),
					'MKD' => __( 'Macedonian denar', 'classic-store'),
					'MMK' => __( 'Burmese kyat', 'classic-store'),
					'MNT' => __( 'Mongolian t&ouml;gr&ouml;g', 'classic-store'),
					'MOP' => __( 'Macanese pataca', 'classic-store'),
					'MRO' => __( 'Mauritanian ouguiya', 'classic-store'),
					'MUR' => __( 'Mauritian rupee', 'classic-store'),
					'MVR' => __( 'Maldivian rufiyaa', 'classic-store'),
					'MWK' => __( 'Malawian kwacha', 'classic-store'),
					'MXN' => __( 'Mexican peso', 'classic-store'),
					'MYR' => __( 'Malaysian ringgit', 'classic-store'),
					'MZN' => __( 'Mozambican metical', 'classic-store'),
					'NAD' => __( 'Namibian dollar', 'classic-store'),
					'NGN' => __( 'Nigerian naira', 'classic-store'),
					'NIO' => __( 'Nicaraguan c&oacute;rdoba', 'classic-store'),
					'NOK' => __( 'Norwegian krone', 'classic-store'),
					'NPR' => __( 'Nepalese rupee', 'classic-store'),
					'NZD' => __( 'New Zealand dollar', 'classic-store'),
					'OMR' => __( 'Omani rial', 'classic-store'),
					'PAB' => __( 'Panamanian balboa', 'classic-store'),
					'PEN' => __( 'Sol', 'classic-store'),
					'PGK' => __( 'Papua New Guinean kina', 'classic-store'),
					'PHP' => __( 'Philippine peso', 'classic-store'),
					'PKR' => __( 'Pakistani rupee', 'classic-store'),
					'PLN' => __( 'Polish z&#x142;oty', 'classic-store'),
					'PRB' => __( 'Transnistrian ruble', 'classic-store'),
					'PYG' => __( 'Paraguayan guaran&iacute;', 'classic-store'),
					'QAR' => __( 'Qatari riyal', 'classic-store'),
					'RON' => __( 'Romanian leu', 'classic-store'),
					'RSD' => __( 'Serbian dinar', 'classic-store'),
					'RUB' => __( 'Russian ruble', 'classic-store'),
					'RWF' => __( 'Rwandan franc', 'classic-store'),
					'SAR' => __( 'Saudi riyal', 'classic-store'),
					'SBD' => __( 'Solomon Islands dollar', 'classic-store'),
					'SCR' => __( 'Seychellois rupee', 'classic-store'),
					'SDG' => __( 'Sudanese pound', 'classic-store'),
					'SEK' => __( 'Swedish krona', 'classic-store'),
					'SGD' => __( 'Singapore dollar', 'classic-store'),
					'SHP' => __( 'Saint Helena pound', 'classic-store'),
					'SLL' => __( 'Sierra Leonean leone', 'classic-store'),
					'SOS' => __( 'Somali shilling', 'classic-store'),
					'SRD' => __( 'Surinamese dollar', 'classic-store'),
					'SSP' => __( 'South Sudanese pound', 'classic-store'),
					'STD' => __( 'S&atilde;o Tom&eacute; and Pr&iacute;ncipe dobra', 'classic-store'),
					'SYP' => __( 'Syrian pound', 'classic-store'),
					'SZL' => __( 'Swazi lilangeni', 'classic-store'),
					'THB' => __( 'Thai baht', 'classic-store'),
					'TJS' => __( 'Tajikistani somoni', 'classic-store'),
					'TMT' => __( 'Turkmenistan manat', 'classic-store'),
					'TND' => __( 'Tunisian dinar', 'classic-store'),
					'TOP' => __( 'Tongan pa&#x2bb;anga', 'classic-store'),
					'TRY' => __( 'Turkish lira', 'classic-store'),
					'TTD' => __( 'Trinidad and Tobago dollar', 'classic-store'),
					'TWD' => __( 'New Taiwan dollar', 'classic-store'),
					'TZS' => __( 'Tanzanian shilling', 'classic-store'),
					'UAH' => __( 'Ukrainian hryvnia', 'classic-store'),
					'UGX' => __( 'Ugandan shilling', 'classic-store'),
					'USD' => __( 'United States (US) dollar', 'classic-store' ),
					'UYU' => __( 'Uruguayan peso', 'classic-store'),
					'UZS' => __( 'Uzbekistani som', 'classic-store'),
					'VEF' => __( 'Venezuelan bol&iacute;var (2008–2018)', 'classic-store' ),
					'VES' => __( 'Venezuelan bol&iacute;var', 'classic-store'),
					'VND' => __( 'Vietnamese &#x111;&#x1ed3;ng', 'classic-store'),
					'VUV' => __( 'Vanuatu vatu', 'classic-store'),
					'WST' => __( 'Samoan t&#x101;l&#x101;', 'classic-store'),
					'XAF' => __( 'Central African CFA franc', 'classic-store'),
					'XCD' => __( 'East Caribbean dollar', 'classic-store'),
					'XOF' => __( 'West African CFA franc', 'classic-store'),
					'XPF' => __( 'CFP franc', 'classic-store'),
					'YER' => __( 'Yemeni rial', 'classic-store'),
					'ZAR' => __( 'South African rand', 'classic-store'),
					'ZMW' => __( 'Zambian kwacha', 'classic-store'),
				)
			)
		);
	}
...
function wc_get_image_size( $image_size ) {
	...
	$size      = ! is_customize_preview() ? wp_cache_get( $cache_key, 'classic-commerce' ) : false;
	...
	if ( is_customize_preview() ) {
		...
		wp_cache_set( $cache_key, $size, 'classic-commerce' );
	}
	return $size;
}
 ...
 *
 * @since WC-2.5.0
 * @since WC-9.3.0 To support shortcodes on other pages besides the main cart page, this returns the current URL if it is the cart page.
 *
...
function wc_get_credit_card_type_label( $type ) {
	...
	$labels = apply_filters(
		'woocommerce_credit_card_type_labels', array(
			'mastercard'       => __( 'MasterCard', 'classic-store'),
			'visa'             => __( 'Visa', 'classic-store'),
			'discover'         => __( 'Discover', 'classic-store'),
			'american express' => __( 'American Express', 'classic-store'),
			'diners'           => __( 'Diners', 'classic-store'),
			'jcb'              => __( 'JCB', 'classic-store'),
		)
	);
...
function wc_get_rounding_precision() {
	...
	/**
	 * Filter the rounding precision for internal CC calculations. This is different from the number of decimals used for display.
	...
	if ( is_array( $implements ) && in_array( 'WC_Logger_Interface', $implements, true ) ) {
		...
		wc_doing_it_wrong(
			...
			sprintf(
				...
				__( 'The class %1$s provided by %2$s filter must implement %3$s.', 'classic-store'),
...
function wc_get_permalink_structure() {
	...
	$permalinks       = wp_parse_args(
		array_filter( $saved_permalinks ), array(
			'product_base'           => _x( 'product', 'slug', 'classic-store'),
			'category_base'          => _x( 'product-category', 'slug', 'classic-store'),
			'tag_base'               => _x( 'product-tag', 'slug', 'classic-store'),
			'attribute_base'         => '',
...
function wc_load_cart() {
    if ( ! did_action( 'before_woocommerce_init' ) || doing_action( 'before_woocommerce_init' ) ) {
		...
		wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%1$s should not be called before the %2$s action.', 'classic-store'), 'wc_load_cart', 'woocommerce_init' ), '3.7' );
		return;
	}
