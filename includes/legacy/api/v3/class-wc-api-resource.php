...
class WC_API_Resource {
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// Validate ID
		if ( empty( $id ) ) {
			return new WP_Error( "woocommerce_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'classic-store'), $type ), array( 'status' => 404 ) );
		}
		...
		if ( 'customer' !== $type ) {
			...
			if ( null === $post ) {
				return new WP_Error( "woocommerce_api_no_{$resource_name}_found", sprintf( __( 'No %1$s found with the ID equal to %2$s', 'classic-store'), $resource_name, $id ), array( 'status' => 404 ) );
			}
			...
			// Validate post type
			if ( $type !== $post_type ) {
				return new WP_Error( "woocommerce_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'classic-store'), $resource_name ), array( 'status' => 404 ) );
			}
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! $this->is_readable( $post ) ) {
						return new WP_Error( "woocommerce_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'classic-store'), $resource_name ), array( 'status' => 401 ) );
					}
					break;

				case 'edit':
					if ( ! $this->is_editable( $post ) ) {
						return new WP_Error( "woocommerce_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'classic-store'), $resource_name ), array( 'status' => 401 ) );
					}
					break;

				case 'delete':
					if ( ! $this->is_deletable( $post ) ) {
						return new WP_Error( "woocommerce_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'classic-store'), $resource_name ), array( 'status' => 401 ) );
					}
					break;
			}
		}
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( 'customer' === $type ) {
			...
			if ( $result ) {
				return array( 'message' => __( 'Permanently deleted customer', 'classic-store') );
			} else {
				return new WP_Error( 'woocommerce_api_cannot_delete_customer', __( 'The customer cannot be deleted', 'classic-store'), array( 'status' => 500 ) );
			}
		} else {
			...
			if ( ! $result ) {
				return new WP_Error( "woocommerce_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'classic-store'), $resource_name ), array( 'status' => 500 ) );
			}

			if ( $force ) {
				return array( 'message' => sprintf( __( 'Permanently deleted %s', 'classic-store'), $resource_name ) );

			} else {

				$this->server->send_status( '202' );

				return array( 'message' => sprintf( __( 'Deleted %s', 'classic-store'), $resource_name ) );
			}
		}
	}
