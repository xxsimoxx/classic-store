...
class WC_API_Products extends WC_API_Resource {
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_categories', __( 'You do not have permission to read product attributes', 'classic-store'), 401 );
			}
	...
	public function get_product_by_sku( $sku, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_sku', __( 'Invalid product SKU', 'classic-store'), 404 );
			}
