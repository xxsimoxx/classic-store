...
class WC_API_Authentication {
	...
	private function perform_ssl_authentication() {
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer key is missing.', 'classic-store'), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer secret is missing.', 'classic-store'), 404 );
		}

		$keys = $this->get_keys_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $keys['consumer_secret'], $consumer_secret ) ) {
			throw new Exception( __( 'Consumer secret is invalid.', 'classic-store'), 401 );
		}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) ) {
				throw new Exception( sprintf( __( '%s parameter is missing', 'classic-store'), $param_name ), 404 );
			}
		}
	...
	private function get_keys_by_consumer_key( $consumer_key ) {
		...
		if ( empty( $keys ) ) {
			throw new Exception( __( 'Consumer key is invalid.', 'classic-store'), 401 );
		}
	...
	private function get_user_by_id( $user_id ) {
		...
		if ( ! $user ) {
			throw new Exception( __( 'API user is invalid', 'classic-store'), 401 );
		}
	...
	private function check_oauth_signature( $keys, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			throw new Exception( __( 'Invalid signature - failed to sort parameters.', 'classic-store'), 401 );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			throw new Exception( __( 'Invalid signature - signature method is invalid.', 'classic-store'), 401 );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			throw new Exception( __( 'Invalid signature - provided signature does not match.', 'classic-store'), 401 );
		}
	}
	...
	private function check_oauth_timestamp_and_nonce( $keys, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			throw new Exception( __( 'Invalid timestamp.', 'classic-store'), 401 );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			throw new Exception( __( 'Invalid nonce - nonce has already been used.', 'classic-store'), 401 );
		}
		...
		switch ( WC()->api->server->method ) {
			...
			case 'GET':
				if ( 'read' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have read permissions.', 'classic-store'), 401 );
				}
				break;
			...
			case 'DELETE':
				if ( 'write' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have write permissions.', 'classic-store'), 401 );
				}
				break;
		}
	}
