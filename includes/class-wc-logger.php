...
class WC_Logger implements WC_Logger_Interface {
				...
				if ( is_object( $handler ) && is_array( $implements ) && in_array( 'WC_Log_Handler_Interface', $implements, true ) ) {
					...
					wc_doing_it_wrong(
						...
						sprintf(
							...
							__( 'The provided handler %1$s does not implement %2$s.', 'classic-store'),
	...
	public function log( $level, $message, $context = array() ) {
		if ( ! WC_Log_Levels::is_valid_level( $level ) ) {
			...
			wc_doing_it_wrong( __METHOD__, sprintf( __( '%1$s was called with an invalid level "%2$s".', 'classic-store'), '<code>WC_Logger::log</code>', $level ), '3.0' );
		}
