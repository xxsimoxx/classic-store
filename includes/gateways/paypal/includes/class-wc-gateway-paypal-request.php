...
class WC_Gateway_Paypal_Request {
	...
	protected function get_shipping_cost_line_item( $order, $force_one_line_item ) {
		...
		if ( $order->get_shipping_total() > 0 && $order->get_shipping_total() < 999.99 && $this->number_format( $order->get_shipping_total() + $order->get_shipping_tax(), $order ) !== $this->number_format( $order->get_total(), $order ) ) {
			...
			$this->add_line_item( sprintf( __( 'Shipping via %s', 'classic-store'), $order->get_shipping_method() ), 1, $this->number_format( $shipping_total, $order ) );
		}
	...
	protected function get_line_item_args_single_item( $order ) {
		...
		$this->add_line_item( $all_items_name ? $all_items_name : __( 'Order', 'classic-store'), 1, $this->number_format( $order->get_total() - $this->round( $order->get_shipping_total() + $order->get_shipping_tax(), $order ), $order ), $order->get_order_number() );
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0.0, $item_number = '' ) {
		...
		$item = apply_filters(
			...
			array(
				'item_name'   => html_entity_decode( wc_trim_string( $item_name ? wp_strip_all_tags( $item_name ) : __( 'Item', 'classic-store'), 127 ), ENT_NOQUOTES, 'UTF-8' ),
