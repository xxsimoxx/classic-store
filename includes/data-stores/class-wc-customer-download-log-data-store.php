...
class WC_Customer_Download_Log_Data_Store implements WC_Customer_Download_Log_Data_Store_Interface {
	...
	public function create( WC_Customer_Download_Log &$download_log ) {
		...
		if ( $result ) {
			...
			wp_die( esc_html__( 'Unable to insert download log entry in database.', 'classic-store') );
		}
	}
	...
	public function read( &$download_log ) {
		...
		if ( ! $download_log->get_id() ) {
			throw new Exception( __( 'Invalid download log: no ID.', 'classic-store') );
		}
		...
		if ( ! $raw_download_log ) {
			throw new Exception( __( 'Invalid download log: not found.', 'classic-store') );
		}
