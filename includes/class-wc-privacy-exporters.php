...
class WC_Privacy_Exporters {
		...
		if ( $user instanceof WP_User ) {
			...
			if ( ! empty( $customer_personal_data ) ) {
				$data_to_export[] = array(
					...
					'group_label'       => __( 'Customer Data', 'classic-store'),
					'group_description' => __( 'User&#8217;s Classic Commerce customer data.', 'classic-store'),
					'item_id'           => 'user',
		...
		if ( 0 < count( $orders ) ) {
			foreach ( $orders as $order ) {
				$data_to_export[] = array(
					...
					'group_label'       => __( 'Orders', 'classic-store'),
					'group_description' => __( 'User&#8217;s Classic Commerce orders data.', 'classic-store'),
		...
		if ( 0 < count( $downloads ) ) {
			foreach ( $downloads as $download ) {
				$data_to_export[] = array(
					...
					'group_label'       => __( 'Purchased Downloads', 'classic-store'),
					'group_description' => __( 'User&#8217;s Classic Commerce purchased downloads data.', 'classic-store'),
				...
				foreach ( $download_logs as $download_log ) {
					$data_to_export[] = array(
						...
						'group_label'       => __( 'Access to Purchased Downloads', 'classic-store' ),
						'group_description' => __( 'User&#8217;s WooCommerce access to purchased downloads data.', 'classic-store' ),
						'item_id'           => 'download-log-' . $download_log->get_id(),
						'data'              => array(
							array(
								'name'  => __( 'Download ID', 'classic-store'),
								'value' => $download_log->get_permission_id(),
							),
							array(
								'name'  => __( 'Timestamp', 'classic-store'),
								'value' => $download_log->get_timestamp(),
							),
							array(
								'name'  => __( 'IP Address', 'classic-store'),
	...
	protected static function get_order_personal_data( $order ) {
		...
		$props_to_export = apply_filters( 'woocommerce_privacy_export_order_personal_data_props', array(
			'order_number'               => __( 'Order Number', 'classic-store'),
			'date_created'               => __( 'Order Date', 'classic-store'),
			'total'                      => __( 'Order Total', 'classic-store'),
			'items'                      => __( 'Items Purchased', 'classic-store'),
			'customer_ip_address'        => __( 'IP Address', 'classic-store'),
			'customer_user_agent'        => __( 'Browser User Agent', 'classic-store'),
			'formatted_billing_address'  => __( 'Billing Address', 'classic-store'),
			'formatted_shipping_address' => __( 'Shipping Address', 'classic-store'),
			'billing_phone'              => __( 'Phone Number', 'classic-store'),
			'billing_email'              => __( 'Email Address', 'classic-store'),
            'shipping_phone'             => __( 'Shipping Phone Number', 'classic-store'),
		), $order );
		...
		$meta_to_export = apply_filters( 'woocommerce_privacy_export_order_personal_data_meta', array(
			'Payer first name'     => __( 'Payer first name', 'classic-store'),
			'Payer last name'      => __( 'Payer last name', 'classic-store'),
			'Payer PayPal address' => __( 'Payer PayPal address', 'classic-store'),
			'Transaction ID'       => __( 'Transaction ID', 'classic-store'),
		) );
	...
	protected static function get_download_personal_data( $download ) {
		$personal_data = array(
			...
			array(
				'name'  => __( 'Order ID', 'classic-store'),
				'value' => $download->get_order_id(),
			),
			array(
				'name'  => __( 'Product', 'classic-store'),
				'value' => get_the_title( $download->get_product_id() ),
			),
			array(
				'name'  => __( 'User email', 'classic-store'),
				'value' => $download->get_user_email(),
			),
			array(
				'name'  => __( 'Downloads remaining', 'classic-store'),
				'value' => $download->get_downloads_remaining(),
			),
			array(
				'name'  => __( 'Download count', 'classic-store'),
				'value' => $download->get_download_count(),
			),
			array(
				'name'  => __( 'Access granted', 'classic-store'),
				'value' => gmdate( 'Y-m-d', $download->get_access_granted( 'edit' )->getTimestamp() ),
			),
			array(
				'name'  => __( 'Access expires', 'classic-store'),
		...
		if ( 0 < count( $tokens ) ) {
			foreach ( $tokens as $token ) {
				$data_to_export[] = array(
					...
					'group_label'       => __( 'Payment Tokens', 'classic-store'),
					'group_description' => __( 'User&#8217;s Classic Commerce payment tokens data.', 'classic-store'),
					'item_id'           => 'token-' . $token->get_id(),
					'data'              => array(
						array(
							'name'  => __( 'Token', 'classic-store'),
