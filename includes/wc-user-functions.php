...
function wc_update_user_last_active( $user_id ) {
	...
	update_user_meta( $user_id, 'wc_last_active', (string) strtotime( gmdate( 'Y-m-d', current_time( 'timestamp', true ) ) ) );
}

/**
 * Translate WC roles using the classic-commerce textdomain.
 *
 * @since 3.7.0
...
function wc_translate_user_roles( $translation, $text, $context, $domain ) {
	...
	if ( 'User role' === $context && 'default' === $domain && in_array( $text, array( 'Shop manager', 'Customer' ), true ) ) {
		return translate_user_role( $text, 'classic-commerce' );
    }
