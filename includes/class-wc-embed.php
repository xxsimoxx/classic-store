...
class WC_Embed {
	...
	public static function product_buttons() {
		...
		if ( $_product->is_type( 'simple' ) && $_product->is_purchasable() && $_product->is_in_stock() ) {
			$buttons[] = sprintf( $button, esc_url( add_query_arg( 'add-to-cart', get_the_ID(), wc_get_cart_url() ) ), esc_html__( 'Buy now', 'classic-store') );
		}

		$buttons[] = sprintf( $button, get_the_permalink(), esc_html__( 'Read more', 'classic-store') );
			...
			<div class="wc-embed-rating">
				<?php
					printf(
						...
						esc_html__( 'Rated %s out of 5', 'classic-store'),
