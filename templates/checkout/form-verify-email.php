 ...
 *
 * This template can be overridden by copying it to yourtheme/classic-commerce/checkout/form-verify-email.php.
 *
...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email" action="<?php echo esc_url( $verify_url ); ?>">
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( 'We were unable to verify the email address you provided. Please try again.', 'classic-commerce' ), 'error' );
	}
	?>
	<p>
		<?php
		printf(
			...
			esc_html__( 'To view this page, you must either %1$slogin%2$s or verify the email address associated with the order.', 'classic-commerce' ),
	...
	<p class="form-row">
		<label for="email"><?php esc_html_e( 'Email address', 'classic-commerce' ); ?>&nbsp;<span class="required">*</span></label>
	...
	<p class="form-row">
		<button type="submit" class="woocommerce-button button" name="verify" value="1">
			<?php esc_html_e( 'Verify', 'classic-commerce' ); ?>
		</button>
	</p>
</form>